...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    // LogPrintf("---nStakeModifierTime = %s\n", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexFrom->GetBlockTime()).c_str());
...
{
    //StintCoin will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake)
{
    ...
    // LogPrintf("CheckProofOfStake() : =----=>CheckStakeKernelHash \n");
